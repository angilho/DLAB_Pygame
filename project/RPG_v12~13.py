...
# 12-1 에너지 이미지
health_ani = [p.image.load("Movement_Animations/heart0.png"), p.image.load("Movement_Animations/heart.png"),
              p.image.load("Movement_Animations/heart2.png"), p.image.load("Movement_Animations/heart3.png"),
              p.image.load("Movement_Animations/heart4.png"), p.image.load("Movement_Animations/heart5.png")]
...
#12-3 에너지 class 만들기
class HealthBar(p.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = p.image.load("Movement_Animations/heart5.png")
...
class Player(p.sprite.Sprite):
    ...
    def player_hit(self):
        if not self.cooldown:
            ...
            # 12-5 플레이어가 hit 일경우 체력 감소
            self.health = self.health - 1
            health.image = health_ani[self.health]

            # 에너지가 0이하 로 떨어졌을 경우 액션
            if self.health <= 0:
                self.kill()
